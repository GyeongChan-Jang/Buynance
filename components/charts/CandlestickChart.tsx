...
const CandlestickChart = ({ data, interval, onIntervalChange, baseAsset, quoteAsset, containerRef }: CandlestickChartProps) => {
    ...
    // 차트 크기 업데이트를 위한 useEffect
    useEffect(() => {
      const updateChartSize = () => {
        if (chartRef.current && containerRef.current) {
          const height = containerRef.current.clientHeight
          chartRef.current.chart.setSize(undefined, height - 70)
        }
      }
